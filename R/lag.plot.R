...
{
    ## Purpose: Plot X_t  vs  X_{t-1} ... X_t  vs X_{t+lags}
    ## -------------------------------------------------------------------------
    ## Arguments: x:	time series (possibly multivariate)
    ##		lags:   maximal lag number --> lags plots per column are done
    ## -------------------------------------------------------------------------
    ## Author: Martin Maechler, Date: 10 Nov 2000, 17:08

##==> put this *into*  the ts package !
##=== think about putting  nrpl.2.mfrow() into base ===> DONE == n2mfrow()
    require(ts)
